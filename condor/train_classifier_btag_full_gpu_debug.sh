...
LOG_DIR="${LOG_DIR:-${PROJECT_DIR}/spanet_output/classifier_btag_full_debug}"
VENV_ACTIVATE="${VENV_ACTIVATE:-${PROJECT_DIR}/SPANet/spanet_Env/bin/activate}"
...
# Debug run: tiny settings so we can validate startup quickly.
python -m spanet.train \
  -of "${OPTIONS_FILE}" \
  --log_dir "${LOG_DIR}" \
  --name "classifier_btag_full_debug" \
